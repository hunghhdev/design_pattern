...
public class CalculatorProxy implements ICalculator {
    ...
    public double add(double first, double second) {
        if (first / 2 + second / 2 >= Double.MAX_VALUE / 2) throw new RuntimeException("Out of range");
        if (first / 2 + second / 2 >= Double.MIN_VALUE / 2) throw new RuntimeException("Out of range");
